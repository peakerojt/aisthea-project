...
const COLUMNS = [
    ...
    { header: 'Template size guide', key: 'sizeGuideTemplateKey', width: 24 },
    { header: 'Fit type', key: 'fitType', width: 18 },
    { header: 'Fit note', key: 'fitNote', width: 28 },
    { header: 'Model cao (cm)', key: 'modelHeightCm', width: 16 },
    { header: 'Model nặng (kg)', key: 'modelWeightKg', width: 16 },
    { header: 'Model mặc size', key: 'modelWearSize', width: 16 },
...
const RowSchema = z.object({
    ...
    sizeGuideTemplateKey: z.string().optional(),
    fitType: z.string().optional(),
    fitNote: z.string().optional(),
    modelHeightCm: z.preprocess((value) => (value === '' || value === null || value === undefined ? undefined : Number(value)), z.number().int().positive().optional()),
    modelWeightKg: z.preprocess((value) => (value === '' || value === null || value === undefined ? undefined : Number(value)), z.number().int().positive().optional()),
    modelWearSize: z.string().optional(),
...
export async function generateTemplate(): Promise<Buffer> {
    ...
    const samples = [
        {
            ...
            category: 'Áo khoác',
            sizeGuideTemplateKey: 'tops-regular',
            fitType: 'regular',
            fitNote: 'Form regular, nên chọn đúng size thường mặc.',
            modelHeightCm: 172,
            modelWeightKg: 52,
            modelWearSize: 'M',
            sku: 'AK-NAM-DEN-S',
    ...
    worksheet.mergeCells(`A${noteRow.number}:R${noteRow.number}`);
    ...
    for (const product of products) {
        ...
        if (product.variants.length === 0) {
            ...
            worksheet.addRow({
                ...
                sizeGuideTemplateKey: product.sizeGuideTemplateKey ?? '',
                fitType: product.fitType ?? '',
                fitNote: product.fitNote ?? '',
                modelHeightCm: product.modelHeightCm ?? '',
                modelWeightKg: product.modelWeightKg ?? '',
                modelWearSize: product.modelWearSize ?? '',
                sku: '',
            ...
            product.variants.forEach((variant, idx) => {
                ...
                const row = {
                    ...
                    sizeGuideTemplateKey: idx === 0 ? (product.sizeGuideTemplateKey ?? '') : '',
                    fitType: idx === 0 ? (product.fitType ?? '') : '',
                    fitNote: idx === 0 ? (product.fitNote ?? '') : '',
                    modelHeightCm: idx === 0 ? (product.modelHeightCm ?? '') : '',
                    modelWeightKg: idx === 0 ? (product.modelWeightKg ?? '') : '',
                    modelWearSize: idx === 0 ? (product.modelWearSize ?? '') : '',
                    sku: variant.sku,
                ...
                if (product) {
                    ...
                    await tx.product.update({
                        ...
                        data: {
                            ...
                            sizeGuideTemplateKey: productInfo.sizeGuideTemplateKey || null,
                            fitType: productInfo.fitType || null,
                            fitNote: productInfo.fitNote || null,
                            modelHeightCm: productInfo.modelHeightCm ? Number(productInfo.modelHeightCm) : null,
                            modelWeightKg: productInfo.modelWeightKg ? Number(productInfo.modelWeightKg) : null,
                            modelWearSize: productInfo.modelWearSize || null,
                        },
                    });
                } else {
                    // CREATE new product

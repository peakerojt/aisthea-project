/* =============================================================
   AISTHEA DATABASE FULL RESET

   Purpose:
   - Build the current schema
   - Apply idempotent schema patches
   - Re-run the destructive seed script

   Warning:
   - Existing seeded business data can be overwritten
   - Use only when you intentionally want a reset

   Run in SSMS:
   1. Create database AISTHEA if it does not exist yet
   2. Enable SQLCMD Mode
   3. Update BasePath below to your local server/database folder
   4. Press F5
   ============================================================= */

/* Only edit BasePath below. Point it to your local server/database folder. */
:setvar BasePath "C:\Users\Administrator\Downloads\OJT\Week3_GIT\AISTHEA-Project\server\database"
:ON ERROR EXIT

USE AISTHEA;
GO

PRINT '=============================================='
PRINT '  AISTHEA DATABASE FULL RESET STARTING...'
PRINT '=============================================='
GO

PRINT '> STEP 1/13 - Base schema';
GO
:r $(BasePath)\01_schema_all.sql
GO

PRINT '> STEP 2/13 - Patch PurchaseOrder receiving fields';
GO
:r $(BasePath)\04_purchase_order_fields.sql
GO

PRINT '> STEP 3/13 - Patch order pricing fields';
GO
:r $(BasePath)\05_order_pricing_fields.sql
GO

PRINT '> STEP 4/13 - Patch shipment provider fields';
GO
:r $(BasePath)\06_shipping_provider_metadata.sql
GO

PRINT '> STEP 5/13 - Patch delivery proof fields';
GO
:r $(BasePath)\07_delivery_proof_metadata.sql
GO

PRINT '> STEP 6/13 - Patch address ward field';
GO
:r $(BasePath)\08_addresses_ward.sql
GO

PRINT '> STEP 7/13 - Patch return request refund status';
GO
:r $(BasePath)\09_return_request_refund_status.sql
GO

PRINT '> STEP 8/13 - Patch return request finance note';
GO
:r $(BasePath)\10_return_request_finance_note.sql
GO

PRINT '> STEP 9/13 - Patch order item economics snapshot';
GO
:r $(BasePath)\11_order_item_economics.sql
GO

PRINT '> STEP 10/13 - Patch item-level return attachments';
GO
:r $(BasePath)\12_return_request_item_attachments.sql
GO

PRINT '> STEP 11/13 - Patch return item reason text';
GO
:r $(BasePath)\13_return_request_item_reason_text.sql
GO

PRINT '> STEP 12/13 - Patch refund bank workflow schema';
GO
:r $(BasePath)\14_refund_bank_workflow.sql
GO

PRINT '> STEP 13/13 - Destructive seed reset';
GO
:r $(BasePath)\03_seed_data_standard_fixed.sql
GO

PRINT '=============================================='
PRINT '  AISTHEA DATABASE FULL RESET COMPLETE!'
PRINT '=============================================='
GO

...
const PaymentQR: React.FC = () => {
    ...
    // Authentic External Bank Logos mirroring the structural lineup using reliable public CDNs
    const banks = [
                ...
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4 border-r border-border-dark pr-4">
                        <div className="bg-white px-2 py-1 flex items-center justify-center rounded">
                            {/* CSS-based VNPAY Logo to guarantee it never breaks */}
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 flex-1 flex flex-col lg:flex-row gap-8 lg:gap-16 justify-center items-center lg:items-start py-6 w-full animate-fade-in-up">
                ...
                <div className="flex-1 flex flex-col items-center w-full max-w-sm">
                    ...
                    <div className="text-center mb-6">
                        <p className="text-gray-400 mb-1 text-sm uppercase tracking-widest">Tổng thanh toán</p>
                        <p className="text-3xl font-black text-white tracking-tight">{new Intl.NumberFormat('vi-VN').format(totalAmount)}<span className="text-xl inline-block align-top underline underline-offset-2 ml-1">đ</span></p>
                    </div>
                    ...
                    <div className="bg-[#fff9e6] border border-[#ffe082] rounded py-3 px-5 w-full flex items-center justify-between shadow-lg shadow-yellow-900/10">
                        <span className="text-gray-800 font-medium text-sm">Mã QR hết hạn sau:</span>
                ...
                <div className="flex-1 w-full max-w-md pt-2">
                    <h2 className="text-lg font-bold text-white mb-5 tracking-wide">Hướng dẫn quét mã QR thanh toán</h2>

                    <ul className="space-y-4 text-gray-300 font-medium mb-8 text-sm">
                        <li className="flex gap-4 items-start">
                            <span className="text-gray-500 font-bold">1.</span>
                            <p><button className="text-blue-400 hover:text-blue-300 transition-colors font-semibold">Tải xuống</button> hoặc chụp ảnh màn hình</p>
                        </li>
                        <li className="flex gap-4 items-start">
                            <span className="text-gray-500 font-bold">2.</span>
                            <p>Mở ứng dụng Mobile Banking của bạn</p>
                        </li>
                        <li className="flex gap-4 items-start">
                        ...
                        <li className="flex gap-4 items-start">
                            ...
                            <p>Chọn ảnh vừa tải hoặc quét trực tiếp để thanh toán</p>
                        </li>
                    </ul>

                    <div>
                        <div className="flex items-center gap-3 mb-4">
                            <h3 className="text-sm font-bold text-white uppercase tracking-wider">Ngân hàng hỗ trợ (VNPay QR)</h3>
                        </div>

                        {/* Smaller Grid */}
                        <div className="bg-white rounded-lg p-4 shadow-xl grid grid-cols-4 gap-4 gap-y-5 border border-gray-200">
                            {banks.map((bank, i) => (
                                <div key={i} className="h-6 hover:scale-110 cursor-pointer flex items-center justify-center transition-transform group relative">
                                    <img
                                        src={bank.url}
                                        alt={bank.name}
                                        ...
                                        onError={(e) => {
                                            ...
                                            target.style.display = 'none'; // Hide broken image gracefully
                                        }}
                                    />
                                    {/* Tooltip */}

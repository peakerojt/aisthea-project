...
export const ProductDetail: React.FC<ProductDetailProps> = ({ setView, setCategory, addToCart, cartCount, product: initialProduct }) => {
  ...
  const defaultProduct = {
    id: 'prod-noir-trench-01',
    name: 'Noir Leather Trench',
    price: 450,
    ref: '892-3301-BLK',
    image: 'https://images.unsplash.com/photo-1550614000-4b9519e02c61?q=80&w=2000&auto=format&fit=crop'
  };

  // Normalize product data (some views use 'img', some 'image', some missing ref/id)
  const product = initialProduct ? {
    ...initialProduct,
    image: initialProduct.image || initialProduct.img || defaultProduct.image,
    ref: initialProduct.ref || `SKU-${Math.floor(Math.random() * 10000)}`,
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen w-full bg-bg-dark">
      {/* Left Gallery */}
      <div className="w-full lg:w-1/2 lg:h-screen lg:sticky lg:top-0 bg-surface-dark relative group">
        <div className="w-full h-[60vh] lg:h-full bg-cover bg-center" style={{ backgroundImage: `url(${product.image})` }}></div>
        <button onClick={() => setView('STORE_COLLECTION')} className="absolute top-6 left-6 p-2 bg-black/50 backdrop-blur-md rounded-full text-white hover:bg-black/70 transition-all">
          <span className="material-symbols-outlined">arrow_back</span>
        </button>
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2">
           {[1,2,3,4].map(i => <div key={i} className={`w-2 h-2 rounded-full ${i === 1 ? 'bg-white' : 'bg-white/30'}`}></div>)}
        </div>
      </div>
        ...
        <div className="flex-1 px-6 py-8 lg:px-16 lg:py-12 flex flex-col gap-10">
           <div className="flex flex-col gap-4">
             <div className="flex justify-between items-start">
               <p className="text-gray-500 text-xs font-medium tracking-widest uppercase">Ref. {product.ref}</p>
             ...
             <p className="text-gray-400 leading-relaxed text-sm max-w-md pt-2">
           ...
           <div className="flex flex-col gap-8 border-t border-border-dark/50 pt-8">
              <div>
                <span className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-3 block">Color — <span className="text-white">{getColorName(selectedColor)}</span></span>
              ...
              <div>
                <div className="flex justify-between items-center mb-3">
                  <span className="text-xs font-bold uppercase tracking-widest text-gray-500">Size — <span className="text-white">{selectedSize}</span></span>
           ...
           <div className="flex flex-col divide-y divide-border-dark border-y border-border-dark mt-4">
             {['Description', 'Shipping & Returns'].map(item => (
               <details key={item} className="group py-4 cursor-pointer">
                 ...
                 <div className="pt-4 text-sm text-gray-400 leading-relaxed"><p>Details placeholder text...</p></div>
               </details>
             ))}
           </div>

           <div className="flex flex-col gap-6 pt-6">
             <h3 className="text-lg font-bold uppercase tracking-tight text-white">Style With</h3>
             <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4">
               {[
                 { name: 'Silver Cuff', price: 120, img: 'https://images.unsplash.com/photo-1611591437281-460bfbe1220a?q=80&w=500&auto=format&fit=crop' },
                 { name: 'Mini Tote', price: 290, img: 'https://images.unsplash.com/photo-1584917865442-de89df76afd3?q=80&w=500&auto=format&fit=crop' },
                 { name: 'Ankle Boot', price: 350, img: 'https://images.unsplash.com/photo-1543163521-1bf539c55dd2?q=80&w=500&auto=format&fit=crop' }
               ].map((item, i) => (
                 <div key={i} className="min-w-[140px] flex flex-col gap-2 cursor-pointer group">
                    <div className="aspect-[3/4] w-full rounded-sm overflow-hidden relative">

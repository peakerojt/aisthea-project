...
export function TrackingDetailPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (mounted) setError(err.message || 'Cannot load tracking');
      } finally {
  ...
  if (!tracking) return <div className="p-4 text-slate-500">Empty tracking data</div>;
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-4 p-4">
      <section className="rounded-xl bg-white p-4 shadow">
        ...
        <p className="mt-2 text-sm text-slate-600">ETA: {tracking.eta ? new Date(tracking.eta).toLocaleString() : 'N/A'}</p>
      </section>

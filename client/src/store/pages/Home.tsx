...
export const Home: React.FC = () => {
  ...
  return (
    <div className="flex flex-col w-full bg-bg-dark font-sans overflow-x-hidden">
      ...
      <section className="w-full bg-bg-dark py-24 px-6 md:px-12 border-b border-white/5">
        <div className="container mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            ...
            <button onClick={() => navigate('/collection')} className="text-xs font-bold uppercase tracking-widest text-white hover:text-primary transition-colors flex items-center gap-2 group">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-12">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="flex flex-col gap-4 animate-pulse">
                  ...
                  <div><div className="h-4 bg-surface-dark w-3/4 mb-2 rounded"></div><div className="h-3 bg-surface-dark w-1/4 rounded"></div></div>
                </div>
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-12">
              {curatedProducts.map((product, i) => (
                <ProductCard
                  ...
                  onClick={() => handleProductClick(curatedRaw[i])}
      ...
      {/* SECTION 3: MEN */}
      <section className="relative h-screen w-full flex items-center cursor-pointer group overflow-hidden border-b border-white/5" onClick={() => navigate('/category/men')}>
        ...
        <div className="relative z-10 container mx-auto px-6 md:px-12 flex items-center">
          <div className="max-w-xl transition-transform duration-700 group-hover:translate-x-4">
            ...
            <p className="text-gray-300 text-lg mb-10 leading-relaxed max-w-md border-l border-white/20 pl-4">Architectural cuts and refined staples, designed for the modern metropolitan life.</p>
            <button className="border border-white text-white px-10 py-4 text-xs font-bold uppercase tracking-[0.2em] hover:bg-white hover:text-black transition-all">Shop Menswear</button>
          </div>
        </div>
      </section>

      {/* SECTION 4: WOMEN */}
      <section className="relative h-screen w-full flex items-center justify-end cursor-pointer group overflow-hidden" onClick={() => navigate('/category/women')}>
        ...
        <div className="relative z-10 container mx-auto px-6 md:px-12 flex flex-col items-end text-right">
          <div className="max-w-xl transition-transform duration-700 group-hover:-translate-x-4 flex flex-col items-end">
            ...
            <div className="h-px w-20 bg-white/30 mb-8"></div>
            <p className="text-gray-200 text-lg mb-10 leading-relaxed border-r border-white/20 pr-4">Fluid silhouettes, flowing structure and precision elegance redefined for the contemporary woman.</p>
            <button className="border border-white text-white px-10 py-4 text-xs font-bold uppercase tracking-[0.2em] hover:bg-white hover:text-black transition-all">Shop Womenswear</button>
          </div>
        </div>
      </section>
      ...
      <section className="relative h-screen w-full flex flex-col items-center justify-center text-center px-6 overflow-hidden bg-[#8C8474]">
        <div className="absolute inset-0 bg-cover bg-center mix-blend-multiply opacity-80" style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1574201635302-388dd92a4c3f?q=80&w=2500)' }}></div>
        ...
        <div className="relative z-10 max-w-4xl animate-fade-in flex flex-col items-center">
          ...
          <button onClick={() => navigate('/stylist')} className="bg-white text-black px-12 py-5 text-xs font-bold uppercase tracking-[0.25em] hover:bg-black hover:text-white transition-colors shadow-2xl">
            View Curated Looks
          </button>
        </div>
      </section>

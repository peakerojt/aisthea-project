      ...
      <div className="px-6 md:px-12 w-full max-w-[1440px] mx-auto h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="relative flex items-center" ref={searchRef}>
            <button onClick={() => setIsSearchActive(!isSearchActive)} className="text-white/90 hover:text-white p-2 transition-colors">
            ...
            {isSearchActive && (searchTerm.trim() !== '' || isSearching || recentSearches.length > 0) && (
                ...
                {isSearching ? (
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
                      ...
                      <button onClick={clearAllRecentSearches} className="text-[10px] font-bold uppercase tracking-widest text-primary hover:text-white transition-colors">Xóa tất cả</button>
                    </div>
                    {recentSearches.map((term, i) => (
                      <div key={i} className="flex items-center justify-between group hover:bg-white/5 transition-colors border-b border-white/5 last:border-none">
                        <button
                          onClick={() => { setSearchTerm(term); saveSearchTerm(term); navigate(`/collection?search=${encodeURIComponent(term)}`); setIsSearchActive(false); }}
                        ...
                        <button onClick={() => removeRecentSearch(term)} className="px-4 py-3 text-gray-500 hover:text-white transition-colors">
                  ...
                  <div className="flex flex-col">
                    {searchResults.map((result) => (
                      <button key={result.productId} onClick={() => handleSearchProduct(result)} className="flex items-center gap-4 p-4 hover:bg-white/5 transition-colors border-b border-white/5 last:border-none text-left w-full">
                        <div className="w-12 h-12 bg-white/5 rounded-sm overflow-hidden flex-shrink-0">
                          {result.images?.[0] && <img src={getCloudinaryProductCard(result.images[0].thumbnailUrl || result.images[0].imageUrl)} alt={result.name} className="w-full h-full object-cover" />}
                        </div>
                        <div className="flex flex-col overflow-hidden">
                          <span className="text-white text-[10px] font-bold truncate uppercase tracking-wider">{result.name}</span>
                          <span className="text-primary text-[10px] font-bold">{new Intl.NumberFormat('vi-VN').format(Number(result.basePrice))}đ</span>
                        </div>
                      </button>
                    ))}
                    <button onClick={handleSearchSubmit} className="p-4 text-center text-[10px] font-black uppercase tracking-[0.2em] text-primary hover:bg-white/5 transition-colors border-t border-white/10">
                      Xem tất cả kết quả
                    </button>
                  </div>
                  ...
                  <div className="p-8 text-center flex flex-col items-center gap-3">
                    ...
                    <button onClick={() => { navigate('/collection'); setIsSearchActive(false); }} className="mt-2 text-primary text-[10px] font-black uppercase tracking-widest hover:underline">
          ...
          <button onClick={() => navigate('/cart')} className="text-white/90 hover:text-white p-2 relative">
